...
void DaemonDlg::OnOk()
{
	...
	{
		MessageBoxW(NULL, L"加载中转服务器配置出错!", L"提示", MB_OK);
		return;
	}

	MessageBoxW(NULL, L"成功生成中转服务器!", L"提示", MB_OK);
...
bool DaemonDlg::GenerateDaemonFileList(std::wstring daemonDir)
{
	...
	MessageBoxW(NULL, L"导出中转服务器出错\r\n指定目录已存在中转服务器!", L"提示", MB_OK);
